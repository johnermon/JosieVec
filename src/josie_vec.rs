...
//Public methods
impl<T> JosieVec<T>{
    ///Constructor for JosieVec
    #[inline(always)]
    pub fn new() -> Self{
        Self::default()
    }
    ///Constructor for JosieVec with preallocated capacity
    ...
    ///Pops last value from JosieVec and optionally returns it.
    pub fn pop(&mut self) -> Option<T>{
        ...
        Some(unsafe{
        self.buf.ptr.add(self.len).read()
        })
    }
    pub fn remove(&mut self, index:usize) -> T{
        ...
        unsafe{
            ...
            copy(self.buf.ptr.as_ptr().add(index+1), self.buf.ptr.as_ptr().add(index), self.len - index);
            //returns
            return out;
        }
    }
    ...
    ///Reserves capacity but tries to allocate efficiently via giving extra space after each realloc.
    #[inline(always)]
    ...
    #[inline(always)]
    ///Reserves capacity for exactly number of elements
    pub fn reserve_exact(&mut self, capacity:usize){
        if capacity > self.buf.cap{
            unsafe{self.realloc_internal(capacity)}
        }
    }
    #[inline(always)]
    ...
    pub fn truncate(&mut self, len:usize){
        if len < self.len{
            unsafe{
                drop_in_place(from_raw_parts_mut(self.buf.ptr.as_ptr().add(len), self.len-len));
            }
            self.len = len;
        }
    }
    ...
    pub fn into_boxed_slice(self) -> Box<[T]>{
        ...
        unsafe{
            ...
            return Box::from_raw(
                slice_from_raw_parts_mut(drop.as_mut_ptr(), drop.len)
            );
       }
    }
...
impl<T> JosieVec<T>{
    ...
    fn grow_amortized(&mut self){
        ...
        unsafe{self.realloc_internal(self.buf.cap<<1);}
    }
        ...
        unsafe{
            ...
            self.buf.ptr = if cap != 0 && self.buf.cap == 0{
                NonNull::new(
                    ...
                    alloc(Layout::array::<T>(cap)
                    .expect("Overflow")) as *mut T)
            ...
            //if you are reallocating to zero, dealloc the current pointer and then create a new non null dangling pointer
                ...
                dealloc(self.buf.ptr.as_ptr() as *mut u8, Layout::array::<T>(self.buf.cap).expect("Overflow"));
...
impl<T> Clone for JosieVec<T> where T:Clone{
    fn clone(&self) -> Self {
...
impl<T> Drop for RawJosieVec<T>{
    fn drop(&mut self){
        unsafe{
            if self.cap != 0{
                dealloc(self.ptr.as_ptr() as *mut u8, Layout::array::<T>(self.cap).expect("Overflow"));
            }
        }
    }
}
